...
enable_big_holder_filter = st.sidebar.checkbox("啟用大戶條件篩選", value=False)
big_holder_level = st.sidebar.selectbox(
    "大戶持股等級定義：",
    ["1,000張以上", "800張以上", "400張以上"],
    index=0
)

min_big_pct = st.sidebar.slider("大戶持股比例高於 (%)", min_value=0, max_value=90, value=50, step=5)
min_weekly_change = st.sidebar.slider("近週持股增加至少 (%)", min_value=-5.0, max_value=5.0, value=0.0, step=0.1)
min_streak_weeks = st.sidebar.slider("大戶連續增加至少 (週)", min_value=0, max_value=8, value=1, step=1)
...
# 5. 從 FinMind API 擷取日 K 線與大戶持股資料
...
def fetch_finmind_big_holders(stock_id, token="", level_str="1,000張以上"):
    ...
    url = "https://api.finmindtrade.com/api/v4/data"
    params = {"dataset": "TaiwanStockHoldingSharesPer", "data_id": stock_id, "start_date": start_date}
    if token:
        params["token"] = token

    # FinMind HoldingPLevel 分級對應:
    # 15: 1,000,001股以上 (1000張以上)
    # 14: 800,001-1,000,000股 (800-1000張)
    # 12, 13: 400,001-800,000股 (400-800張)
    ...
    try:
        ...
        if data.get("status") == 200 and data.get("data"):
            ...
            if len(df_weekly) < 2:
                return {"latest_pct": round(latest_pct, 2), "weekly_change": 0.0, "streak": 0}
...
if st.button(f"🚀 開始 FinMind 數據掃描 ({len(all_stocks)} 檔標的)", use_container_width=True):
    raw_results = []
    ...
    for idx, s in enumerate(all_stocks):
        ...
        big_holder_info = fetch_finmind_big_holders(code, token=finmind_token, level_str=big_holder_level)
    ...
    if len(raw_results) > 0:
        status_text.success(f"🎉 FinMind 資料庫掃描完成！成功分析 {len(raw_results)} 檔股票技術面與大戶籌碼。")
...
if st.session_state.scan_data is not None and not st.session_state.scan_data.empty:
    ...
    # 格式化連續週數顯示
    def format_streak(s):
        if s > 0: return f"連增 {s} 週 🟢"
        elif s < 0: return f"連減 {abs(s)} 週 🔴"
        else: return "持平 ⚪"
    ...
    col_order = ["股票代號", "股票名稱", "收盤價", f"K({kd_period})", f"D({kd_period})", f"RSI({rsi_short})", f"RSI({rsi_long})", f"大戶比例({big_holder_level})", "近週持股增減(%)", "連續增減週數", "狀態"]
    ...
    st.info("💡 請點擊上方『🚀 開始 FinMind 數據掃描』按鈕載入最新技術面與大戶籌碼數據。")
